...
AF_num<-2			#1:BQ, 2:Proposed, 3:SABBa
...
post_mean_tilde_function<-function(x)
	...
	################## Acquisition function ##############################
		...
		result<- numeric(EHVI_sample_num)
			...
			{
			c<-compute_e_accuracy(rbind(sample[i,]),predicted_PF)
				if(c<=0)
				{
				result[i]<-0
				} else {
				D<- rbind(predicted_PF,sample[i,])
				...
				result[i]<-D_PF_PHV-predicted_PF_PHV
		...
		S<-rbind(rbind(input[,1:2]),new_input_x)
		...
		set.seed(((seed-1)*total_seed_num+w)*1000+ww)
		ucb_lcb_sample_w1<- runif(2,w1_min,w1_max)
		ucb_lcb_sample_w2<- runif(2,w2_min,w2_max)
