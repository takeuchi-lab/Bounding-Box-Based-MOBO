...
AF_num<-1			#1:MVA, 2:Proposed, 3:SABBa
...
k_vec_mat<-  matrix(numeric(grid_num^6*iteration_num),grid_num^6,iteration_num)
	...
	k_vec_mat[,ww]<- apply(cbind(new_input[1],new_input[2],new_input[3],new_input[4],new_input[5],new_input[6],all_x1x2x3w1w2w3),1,compute_kernel)	
	
	k_vec_mat_1_ww<-cbind(k_vec_mat[,1:ww])
	k_vec_mat_C_inv<- k_vec_mat_1_ww%*%C_inv
	...
	post_mean_mat<-  k_vec_mat_C_inv%*%Y
	post_var_mat<-  abs(sigmaf-rowSums(k_vec_mat_C_inv*k_vec_mat_1_ww))
	...
	################## Acquisition function ##############################
		...
		lcb_F1<-  colSums(matrix(l_mat,grid_num^3,grid_num^3)*all_w1w2w3_prob)
		ucb_F1<-  colSums(matrix(u_mat,grid_num^3,grid_num^3)*all_w1w2w3_prob)
		
		l_tilde<- as.vector(l_mat)-ucb_F1%x%rep(1,grid_num^3)
		u_tilde<- as.vector(u_mat)-lcb_F1%x%rep(1,grid_num^3)
		...
		ucb_F2<- -sqrt(abs(colSums(matrix(l_tilde_sq_all,grid_num^3,grid_num^3)*all_w1w2w3_prob)))
		lcb_F2<- -sqrt(abs(colSums(matrix(u_tilde_sq_all,grid_num^3,grid_num^3)*all_w1w2w3_prob)))




		LCB_mat<-cbind(lcb_F1,lcb_F2)
		UCB_mat<-cbind(ucb_F1,ucb_F2)
		...
		AF_X<-rowSums((UCB_mat-LCB_mat)^2)*(AF_X>=0)
		...
		reported_Y_mat<-t(apply(rbind(all_x1x2x3[estimated_x_num,]),1,true_risk_function))
		...
		inference_e_accuracy2<- c(inference_e_accuracy2,max(compute_e_accuracy2(true_PF,reported_Y_mat),0))
		...
		#inference_e_accuracy2<- c(inference_e_accuracy2,max(compute_e_accuracy2(true_PF,inference_predicted_PF),0))
		inference_e_accuracy2_mat[w,ww]<-	max(compute_e_accuracy2(true_PF,inference_predicted_PF),0)
		} else {
