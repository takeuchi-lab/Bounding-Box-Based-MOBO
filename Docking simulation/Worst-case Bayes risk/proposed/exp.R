...
predicted_PF_cell<-create_cells(predicted_PF,ref_point,apply(predicted_PF,2,max),c(1:length(predicted_PF[,1])))
	...
	if(setting_ID==1)
	{
		...
		{
		apply(rbind(lcb_mat[info[,1]==i,]),2,max)
		}

		compute_UCB<-function(i)
		{
		apply(rbind(ucb_mat[info[,1]==i,]),2,max)
		}
	}
	...
	if(setting_ID==2)
	{
		compute_LCB<-function(i)
		{
		apply(rbind(lcb_mat[info[,1]==i,]),2,min)
		}

		compute_UCB<-function(i)
		{
		apply(rbind(ucb_mat[info[,1]==i,]),2,min)
		}
	}
	...
	if(setting_ID==4)
	{
		compute_LCB<-function(i)
		{
		apply(rbind(lcb_mat[info[,1]==i,]),2,compute_DR)
		}

		compute_UCB<-function(i)
		{
		apply(rbind(ucb_mat[info[,1]==i,]),2,compute_DR)
		}
	}
	...
	##########################################
		...
		LCB_PF<-rbind(compute_PF(LCB_mat)[,1:obj_dim])
		AFx<-compute_dist(UCB_mat,LCB_PF)
		next_design<- order(AFx,decreasing=T)[1]
		AFw<-rowSums(sqrt(rbind(post_var_mat[info[,1]==next_design,])))
		next_env<- order(AFw,decreasing=T)[1]

		new_X_num<- c(1:total_input_num)[((info[,1]==next_design)& (info[,2]==next_env))]
