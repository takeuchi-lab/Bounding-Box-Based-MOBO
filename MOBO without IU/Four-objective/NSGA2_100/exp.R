...
y<-a[2]
	...
	f3<- (-1*( (x^2+y-11)^2+(x+y^2-7)^2 )-(-136.71))/sqrt(12503.63)

	f4<- (-1*( sin(x+y)+(x-y)^2-1.5*x+2.5*y+1 )-(-17.67))/sqrt(460.573)
...
for(w in 1:total_seed_num)
{
	...
	results <- nsga2R(fn=negative_true_function, varNo=2, objDim=4, lowerBounds=c(lower,lower), upperBounds=c(upper,upper),
	popSize=100, tourSize=2, generations=ww, cprob=0.9, XoverDistIdx=20, mprob=0.1,MuDistIdx=3)
